...
@Composable
fun ArticlesScreen(onAboutButtonClick:() -> Unit,
    ...
    val articleState = articlesViewModel.articlesState.collectAsState()
     ...
     Column {
         ...
         if(articleState.value.error!=null)
             ErrorMessage(articleState.value.error.toString())
...
fun ArticleItemView(article: Article){
    ...
    Column(modifier = Modifier
        ...
        Spacer(modifier = Modifier.height(4.dp))
        Text(text = article.date, style = TextStyle(color = Color.Gray), modifier = Modifier.align(Alignment.End))
        Spacer(modifier = Modifier.height(4.dp))
